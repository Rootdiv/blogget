# Blogget

Курс "React-разработчик" с 05 сентября 2022 до 06 ноября 2022 года.
+ [Урок 6. Работа с API, Авторизация](https://github.com/Rootdiv/blogget/tree/lesson06)
+ [Урок 6. Работа с API, Авторизация. Кастомный хук useAuth](https://github.com/Rootdiv/blogget/tree/lesson06_extra)
+ [Урок 7. Prop drill, Context](https://github.com/Rootdiv/blogget/tree/lesson07)
+ [Урок 8. Portals & Refs](https://github.com/Rootdiv/blogget/tree/lesson08)
+ [Урок 8. Portals & Refs. Фокус на форму через рефы](https://github.com/Rootdiv/blogget/tree/lesson08_extra)
+ [Урок 9. Знакомство с Redux](https://github.com/Rootdiv/blogget/tree/lesson09)
+ [Урок 10. React и Redux DevTools, combineReducer, Middleware, Redux-Thunk](https://github.com/Rootdiv/blogget/tree/lesson10)
+ [Урок 10. React и Redux DevTools, combineReducer, Middleware, Redux-Thunk. Сообщение об ошибке через порталы](https://github.com/Rootdiv/blogget/tree/lesson10_extra)
+ [Урок 11. React Роутинг + бонус Бесконечная прокрутка](https://github.com/Rootdiv/blogget/tree/lesson11)
+ [Урок 11. React Роутинг + бонус Бесконечная прокрутка. Автозагрузка 2-х страниц и далее кнопкой "Загрузить ещё"](https://github.com/Rootdiv/blogget/tree/lesson11_extra)
+ [Урок 12. Современный Redux (Redux Toolkit)](https://github.com/Rootdiv/blogget/tree/lesson12)
